...
// Read CSS from external file (shared design system with website)
let cssPath = Path.Combine(scriptDir, "dashboard.css")
let cssContent =
    if File.Exists(cssPath) then File.ReadAllText(cssPath)
    else
        eprintfn "[WARN] dashboard.css not found at %s — using minimal fallback" cssPath
...
a "</head>"
a "<body>"
a "<div class=\"dashboard-header\">"
a "<div class=\"dashboard-brand\">"
a "<a class=\"logo\" href=\"https://nimblesite.github.io/AgentPMOWorkflow\" target=\"_blank\" rel=\"noopener noreferrer\"><span class=\"logo-mark\">▶</span> Agent PMO</a>"
a "<h1>Dashboard</h1>"
a "</div>"
a "<button class=\"theme-toggle\" onclick=\"toggleTheme()\" title=\"Toggle theme\">☀️</button>"
a "</div>"
...
a "function toggleTheme() {"
a "  var html = document.documentElement;"
a "  var cur = html.getAttribute('data-theme') || 'dark';"
a "  var next = cur === 'dark' ? 'light' : 'dark';"
a "  html.setAttribute('data-theme', next);"
a "  localStorage.setItem('dashboard-theme', next);"
a "  document.querySelector('.theme-toggle').textContent = next === 'dark' ? '☀️' : '🌙';"
a "}"
a "(function() {"
a "  var t = localStorage.getItem('dashboard-theme') || 'dark';"
a "  document.documentElement.setAttribute('data-theme', t);"
a "  var btn = document.querySelector('.theme-toggle');"
a "  if (btn) btn.textContent = t === 'dark' ? '☀️' : '🌙';"
a "})();"
a "</script>"
